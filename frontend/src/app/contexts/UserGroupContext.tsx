...
export const UserGroupsProvider = ({ children }: PropsWithChildren): ReactNode => {
  ...
  const updateGroups = async (forceUpdate: boolean = false) => {
    if (!forceUpdate && lastFetch.current && (Date.now() - lastFetch.current.getTime()) < fetchDelay * 1000) {
      return;
    }
    lastFetch.current = new Date();

    setLoadingGroups(true);
    const groupsApi = await withFetch<null, Group[]>({method: 'GET', resource: '/group/', redirectOnAuthFail: true});
    ...
    if (groupsApi.error) {
      ...
      notifications.show({
        ...
        message: 'There was an error getting your groups. Please try again later.',
        color: 'red'
      })
  ...
  return (
    <UserGroupsContext.Provider value={useMemo(() => ({groups: userGroups, updateGroups, groupsError: groupsError, groupsLoading: loadingGroups}), [userGroups, groupsError, loadingGroups])}>
      {children}
