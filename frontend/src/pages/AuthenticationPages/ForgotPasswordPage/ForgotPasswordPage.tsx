...
const ForgotPasswordPage = () => {
  ...
  const form = useForm<ForgotPasswordForm>({
    ...
    validate: {
      email: (value) =>
        /^\S+@\S+$/.test(value) ? null : "The email specified is not valid",
    },
  });
  ...
  if (emailSent) {
    return (
      <Box>
        <Stack gap="sm">
          <Text size="sm">
            If an account matching the email address you provided exists, a
            password recovery email has been sent. Please check your inbox and
  ...
  return (
    <Box>
      ...
      <ErrorDisplay
        title="An error occured sending the recovery email"
        error={sendEmailError}
      />
      <form onSubmit={form.onSubmit(requestRecoveryEmail)}>
        <Stack gap="sm">
          <Text size="sm">
            Enter your email address below and if it matches an existing
            account, we'll send you an email with instructions to reset your
            password.
          </Text>
          <TextInput
            label="Email"
