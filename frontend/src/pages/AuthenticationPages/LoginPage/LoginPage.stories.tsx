...
SuccessfulLogin.test("shows validation errors on empty submit", async ({ canvas }) => {
  const signInButton = await canvas.getByRole("button", { name: "Sign in" });
...
SuccessfulLogin.test("successful login redirects to dashboard", async ({ canvas, userEvent }) => {
  const emailInput = await canvas.getByLabelText("Email");
  const passwordInput = await canvas.getByLabelText("Password");
  const signInButton = await canvas.getByRole("button", { name: "Sign in" });
...
UnsuccessfulLogin.test("unsuccessful login shows error message", async ({ canvas, userEvent }) => {
  const emailInput = await canvas.getByLabelText("Email");
  const passwordInput = await canvas.getByLabelText("Password");
  const signInButton = await canvas.getByRole("button", { name: "Sign in" });
  ...
  const errorMessage = await canvas.findByText("Invalid email address or password");
