...
const VerifyAccountPage = () => {
  ...
  const { data, error, loading, submitFn } = useREST<VerifyForm, null>(
    "POST",
    `/auth/verify/${searchParams.get("user_id")}/`,
  );
  ...
  return (
    <Box>
      ...
      {data && !error && !loading && (
        <Box>
          Your account has been successfully verified! Please continue to the
          login page.
      ...
      {loading && <Box>Verifying your account, please wait...</Box>}
    </Box>
  );
};
