...
export const SuccessfulCreation = meta.story({
  decorators: [
    (Story) => {
      ...
      fetchMock.post("end:/auth/register/", { status: 204 }, { delay: 500 });
...
export const UnsuccessfulCreation = meta.story({
  decorators: [
    (Story) => {
      ...
      fetchMock.post(
        "end:/auth/register/",
        { status: 409, body: { detail: "Email already exists" } },
        { delay: 500 },
      );
...
SuccessfulCreation.test("renders registration form", async ({ canvas }) => {
  const firstNameInput = await canvas.getByRole("textbox", {
    name: /first name/i,
  });
  const lastNameInput = await canvas.getByRole("textbox", {
    name: /last name/i,
  });
...
SuccessfulCreation.test(
  ...
  async ({ canvas, userEvent }) => {
    ...
    const errorMessage = await canvas.findByText(
      /the email specified is not valid/i,
    );
    expect(errorMessage).toBeInTheDocument();
  },
);
...
SuccessfulCreation.test(
  "checks first and last names are present",
  async ({ canvas, userEvent }) => {
    const emailInput = await canvas.getByRole("textbox", { name: /email/i });
    const passwordInput = await canvas.getByText("Password");
    const verifyPasswordInput = await canvas.getByText("Verify Password");
    ...
    const firstNameError = await canvas.findByText(
